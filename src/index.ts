...
app.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const body: CreateNoteReq = req.body;
    try {
      // RPC call supabase
      let {
        data,
        error,
      }: { data: CreateNoteRes[] | null; error: PostgrestError | null } =
        await supabase.rpc("createnote", {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const {
        data,
        error,
      }: { data: GetNotesRes[] | null; error: PostgrestError | null } =
        await supabase
...
app.post(
  "/updateNote",
  async (req: Request, res: Response, next: NextFunction) => {
    const body: UpdateNoteReq = req.body;
    try {
      const { data, error } = await supabase
        .from("user_note_assoc")
